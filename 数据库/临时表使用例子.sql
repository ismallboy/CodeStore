...
/****** Object:  StoredProcedure [dbo].[SP_MS_ActivityInfoTotalReport]    Script Date: 12/03/2015 16:38:29 ******/
SET ANSI_NULLS ON
GO
...
--求活动实际参与人数
if not exists(select * from tempdb..sysobjects where id=object_id('tempdb..#REAL_JOIN_NUMBER'))
BEGIN
	select actualJoinNumber.RECORD_ID, actualJoinNumber.realJoinNumber
		into #REAL_JOIN_NUMBER
	from  
	(
...
IF NOT EXISTS(SELECT * FROM tempdb..sysobjects WHERE id=OBJECT_ID('tempdb..#DETAIL_CODE_AVG_BUDEGE'))
BEGIN
	...
	WHERE (ISNULL(@beginTime,'') = '' OR realActivityInfo.START_TIME >= @beginTime) AND (ISNULL(@endTime,'') = '' OR realActivityInfo.START_TIME <= @endTime)
		AND (ISNULL(@activityKind,'') = '' OR realActivityInfo.MEETING_KIND_CODE >= @activityKind)AND msApplyInfo.FIRST_END_DATE IS NOT NULL AND realActivityInfo.STATUS = 1--取已经结束的活动的金额和人数
