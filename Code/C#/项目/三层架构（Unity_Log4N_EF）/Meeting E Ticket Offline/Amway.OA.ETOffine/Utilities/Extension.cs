...
namespace Amway.OA.ETOffine
{
    ...
    public static class StringExtension
    {
        ...
        public static string CPadLeft(this string obj, int count, string space = "&nbsp;")
        {
            int byte_len = Encoding.Default.GetBytes(obj).Length;

            if (byte_len > count && count > 4)
            {
            ...
            {
                int patchCount = (int)((count - byte_len)*1.75);
                for (int i = 0; i < patchCount; i++)
                {
                    obj += space;
                }
            }
            return obj + space + space;
        }

        public static string CPadRight(this string obj, int count, string space = "&nbsp;")
        {
